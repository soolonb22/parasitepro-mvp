...
class SpeechEngine {
  ...
  static getBestVoice(): SpeechSynthesisVoice | null {
    ...
    const pick = (fn: any) => this.voices.find(fn) ?? null;
    return (
      pick((v: any) => /junior/i.test(v.name)) ||
      pick((v: any) => /oliver/i.test(v.name) && v.lang.startsWith('en')) ||
      pick((v: any) => v.lang === 'en-AU') ||
      pick((v: any) => v.lang.startsWith('en')) ||
      null
    );
  }
...
const LANDING_SCRIPT = [
  { text: "G'day! I'm PARA — your personal guide to ParasitePro! Found something weird and not sure what it is? You are in exactly the right place. Let me show you what we're all about!", mood: 'waving' as Mood, card: { icon: '👋', title: 'Welcome to ParasitePro', desc: 'notworms.com · Made for Australians' } },
  { text: "ParasitePro uses AI to visually analyse photos you upload — stool samples, skin presentations, weird rashes, suspicious specimens. Our AI compares what you're seeing to thousands of documented cases and gives you a structured educational report in under 60 seconds!", mood: 'talking' as Mood, card: { icon: '🔬', title: 'AI Visual Analysis in ~60s', desc: 'Stool · Skin · Environmental · Microscopy' } },
  { text: "Super important — and I say this every single time because I genuinely care: ParasitePro is an educational reference tool. It is NOT a medical diagnosis. Everything I show you helps you understand what you might be looking at. Always take our reports to your GP to confirm. If something feels urgent right now, call 000 immediately. No hesitation!", mood: 'concerned' as Mood, card: { icon: '⚠️', title: 'Educational Tool Only', desc: 'Not a diagnosis · Always see your GP · Emergency: 000' } },
