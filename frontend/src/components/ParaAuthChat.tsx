 ...
 *
 * Audio strategy:
 *  - Greeting: video's own embedded audio (unmuted on first tap)
 *  - Q&A / free-form: video muted + AudioEngine (ElevenLabs MP3 or Web Speech)
 *
 * Desktop: full left-panel with large video, speech bubble, chips, free-form input
 * Mobile:  compact banner, tap to expand Q&A + input
 */
...
/* ── Ref-controlled video that unmutes on first tap ──────────────────────── */
function ParaVideo({
  speaking,
  onGreetingEnd,
  videoRef,
...
export function ParaAuthPanel({ mode }: { mode: 'signup' | 'login' }) {
  ...
  // greeting finished — switch video to looping SIGNUP_2 (muted talking loop)
  ...
  /* Play video with audio on first tap ─────────────────── */
  function unlock() {
    if (unlocked) return;
    setUnlocked(true);
    ...
    if (!v) return;
    v.muted = false;
    v.loop = false;          // play SIGNUP_1 once, then switch to loop
    v.currentTime = 0;
    v.play().catch(() => {
      // Video has no audio track — fall back to AudioEngine
      v.muted = true;
      v.loop = true;
      speakViaEngine(mode === 'login' ? GREETING_LOGIN : GREETING_SIGNUP);
    });
    setSpeaking(true);
  }

  /* Called when SIGNUP_1 finishes playing ─────────────── */
  function handleGreetingEnd() {
    setSpeaking(false);
    setGreetingDone(true);
    // Switch to SIGNUP_2 looping muted background
    const v = vidRef.current;
    if (!v) return;
    v.muted = true;
    v.loop = true;
  ...
  /* AudioEngine speak (for Q&A where video is muted) ──── */
  function speakViaEngine(t: string, onDone?: () => void) {
    if (audioMuted) { onDone?.(); return; }
  ...
  function handleQ(qa: QA) {
    ...
    // Ensure video is in muted loop mode during Q&A
    const v = vidRef.current;
    ...
    setTimeout(() => {
      ...
      speakViaEngine(qa.a, () => setActiveQ(null));
    }, 500);
  }
  ...
  async function handleFreeform() {
    ...
    try {
      ...
      speakViaEngine(reply, () => setActiveQ(null));
    } catch {
      setThinking(false);
      setText("Sorry, I'm having a bit of trouble connecting right now. Try one of the quick questions, or email support@notworms.com!");
      setActiveQ(null);
    } finally {
      setSending(false);
    }
  ...
  return (
    ...
    >
      {/* ── Video ─────────────────────────────────────────── */}
      <ParaVideo speaking={speaking} onGreetingEnd={handleGreetingEnd} videoRef={vidRef} />
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
          ...
          <button
            ...
            style={{ background:'none', border:'none', cursor:'pointer',
              color: audioMuted ? '#475569' : '#0d9488', fontSize:14, padding:2, lineHeight:1 }}
            title={audioMuted ? 'Unmute PARA' : 'Mute PARA'}
          >
            {audioMuted ? '🔇' : '🔊'}
          </button>
        </div>
        ...
        {!unlocked && (
          <div style={{ marginTop:10, animation:'pac-pulse 2s infinite' }}>
        ...
        {QA_LIST.map(qa => (
          <button
            ...
            style={{
              ...
              background: activeQ === qa.q ? 'rgba(13,148,136,0.18)' : 'rgba(13,148,136,0.06)',
          ...
          <button
            ...
            style={{
              ...
              background: input.trim() && !sending && !thinking
                ? 'linear-gradient(135deg,#0d9488,#0891b2)' : 'rgba(13,148,136,0.1)',
...
/* ══════════════════════════════════════════════════════════
   MOBILE — compact banner with expandable Q&A
══════════════════════════════════════════════════════════ */
export function ParaAuthBanner({ mode, promoCode }: { mode: 'signup' | 'login'; promoCode?: string }) {
  ...
  function handleTap() {
    ...
    if (!unlocked) {
      ...
      if (v) {
        ...
        v.currentTime = 0;
        v.play().catch(() => {
          // No audio track — fall back to engine
          v.muted = true;
          v.loop = true;
          speakViaEngine(greeting);
        });
      } else {
        speakViaEngine(greeting);
      }
  ...
  return (
    <div style={{
      ...
      {/* Top row */}
      <div onClick={handleTap} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', cursor:'pointer' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4, flexShrink:0 }}>
          <button
            onClick={e => { e.stopPropagation(); const nm=!audioMuted; setAudioMuted(nm); if(nm){AudioEngine.cancel();setSpeaking(false);} }}
            style={{ background:'none', border:'none', cursor:'pointer', color: audioMuted ? '#475569' : '#0d9488', fontSize:13, padding:2, lineHeight:1 }}
          >{audioMuted ? '🔇' : '🔊'}</button>
          <span style={{ color:'var(--text-muted)', fontSize:11, display:'inline-block',
            transition:'transform 0.2s', transform: expanded ? 'rotate(180deg)' : 'none' }}>▼</span>
        </div>
      </div>

      {/* Expanded Q&A + free-form */}
      {expanded && (
        <div style={{ padding:'0 14px 14px', borderTop:'1px solid rgba(13,148,136,0.12)', animation:'pac-fadein 0.2s ease' }}>
          <p style={{ margin:'12px 0 8px', fontSize:11, color:'var(--text-muted)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em' }}>
            Quick questions
          </p>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {QA_LIST.map(qa => (
              <button key={qa.q} onClick={() => handleQ(qa)} style={{
                ...
                borderRadius:9, fontSize:12.5, color: activeQ === qa.q ? '#2dd4bf' : 'var(--text-secondary)',
                cursor:'pointer', transition:'all 0.15s', fontWeight: activeQ === qa.q ? 600 : 400,
              }}>
                {activeQ === qa.q ? '▶ ' : ''}{qa.q}
              </button>
          ...
          <p style={{ margin:'12px 0 6px', fontSize:11, color:'var(--text-muted)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em' }}>
            Ask me anything
          </p>
          <div style={{ display:'flex', gap:6 }}>
            <input
              ...
              disabled={sending || thinking}
              style={{ flex:1, padding:'8px 11px', background:'rgba(13,148,136,0.06)',
                border:'1px solid rgba(13,148,136,0.22)', borderRadius:9, fontSize:12.5,
                color:'var(--text-primary)', outline:'none' }}
            />
            <button onClick={handleFreeform} disabled={!input.trim() || sending || thinking} style={{
              padding:'8px 12px', borderRadius:9, border:'none',
              background: input.trim() && !sending && !thinking ? 'linear-gradient(135deg,#0d9488,#0891b2)' : 'rgba(13,148,136,0.1)',
              color: input.trim() && !sending && !thinking ? 'white' : '#475569',
              cursor: input.trim() && !sending && !thinking ? 'pointer' : 'not-allowed',
