...
const AdminPage = () => {
  ...
  const checkAdminAccess = async () => {
    try {
      const res = await axios.get(getApiUrl('/admin/check'), {
  ...
  const searchUser = async (email) => {
    ...
    try {
      const res = await axios.get(getApiUrl(`/admin/search-user?email=${encodeURIComponent(email)}`), {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleGrantCredits = async (e) => {
    ...
    setGrantResult(null);
    try {
      const res = await axios.post(getApiUrl('/admin/grant-credits'), {
        email: grantEmail,
