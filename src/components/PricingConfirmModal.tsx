...
const PricingConfirmModal: React.FC<PricingConfirmModalProps> = ({
  isOpen,
  imagePreview,
  creditBalance,
  onConfirm,
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl max-w-sm w-full shadow-2xl">
        <div className="flex items-center justify-between p-5 pb-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white">Confirm Analysis</h2>
          </div>
          <button onClick={onCancel} className="p-1 hover:bg-gray-700 rounded-lg transition-colors" aria-label="Cancel">
            <X size={18} className="text-gray-400" />
          </button>
        </div>
        <div className="p-5 space-y-4">
          {imagePreview && (
            <img src={imagePreview} alt="Image to analyse" className="w-full h-40 object-cover rounded-xl bg-gray-900" />
          )}
          ...
          {remainingAfter >= 0 && (
            <div className="flex items-center justify-between bg-gray-700 rounded-lg p-3 -mt-2">
              <span className="text-gray-400 text-sm">After analysis</span>
              <span className={`font-semibold ${remainingAfter === 0 ? 'text-orange-400' : 'text-green-400'}`}>{remainingAfter} credits</span>
            </div>
          )}
          {remainingAfter === 0 && (
            <div className="flex items-start gap-2 bg-orange-900 border border-orange-700 rounded-lg p-3">
              <AlertCircle size={16} className="text-orange-400 mt-0.5 flex-shrink-0" />
              <p className="text-orange-200 text-xs">This will use your last credit. You can purchase more anytime from the pricing page.</p>
            </div>
          )}
