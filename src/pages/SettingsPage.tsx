...
const SettingsPage = () => {
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: () => void }) => (
    <button onClick={onChange} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${value ? 'bg-blue-600' : 'bg-gray-600'}`}>
    ...
    <div className="max-w-2xl mx-auto p-4 py-8">
      ...
      <div className="space-y-5">
        <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            <label className="flex items-center justify-between cursor-pointer">
              <div><p className="text-white text-sm font-medium">Email notifications</p><p className="text-gray-400 text-xs">Receive an email when analysis results are ready</p></div>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            <p className="text-gray-400 text-sm">Your images are encrypted and never shared without consent.</p>
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={aiImprovement} onChange={(e) => setAiImprovement(e.target.checked)} className="mt-0.5 w-4 h-4 accent-blue-500" />
              <div><p className="text-white text-sm font-medium">Help improve the AI</p><p className="text-gray-400 text-xs">Allow anonymised images to improve the detection model</p></div>
            </label>
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={research} onChange={(e) => setResearch(e.target.checked)} className="mt-0.5 w-4 h-4 accent-blue-500" />
              <div><p className="text-white text-sm font-medium">Contribute to research</p><p className="text-gray-400 text-xs">Share anonymised data with Australian parasitology researchers</p></div>
            </label>
            ...
            <div className="border-t border-gray-700 pt-4">
              <p className="text-gray-400 text-xs">We comply with the <a href="https://www.oaic.gov.au/privacy/the-privacy-act" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Australian Privacy Principles</a>. Read our <a href="/privacy" className="text-blue-400 hover:underline">Privacy Policy</a>.</p>
            </div>
          </div>
        </div>
        <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
          ...
          <div className="p-5 space-y-3">
            ...
            <button onClick={() => { logout(); navigate('/login'); }} className="w-full flex items-center gap-2 px-4 py-3 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-xl transition-colors"><LogOut size={18} /><span className="text-sm">Sign Out</span></button>
          </div>
        </div>
        <div className="bg-gray-800 border border-red-900 rounded-xl overflow-hidden">
          ...
          <div className="p-5">
            <p className="text-gray-400 text-sm mb-4">Permanently delete your account, all images, analysis history, and personal data. This cannot be undone.</p>
            {!showDeleteConfirm ? (
              ...
              <div className="space-y-3">
                <div className="bg-red-900 border border-red-700 rounded-lg p-3 flex items-start gap-2"><AlertTriangle size={16} className="text-red-400 mt-0.5 flex-shrink-0" /><p className="text-red-200 text-sm">This will permanently delete all your data. <strong>This cannot be undone.</strong></p></div>
                <div>
                  <label className="block text-gray-400 text-xs mb-1.5">Type <strong className="text-white">DELETE</strong> to confirm</label>
                  <input type="text" value={deleteInput} onChange={(e) => setDeleteInput(e.target.value)} placeholder="DELETE" className="w-full px-3 py-2 bg-gray-700 border border-red-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm" />
                </div>
