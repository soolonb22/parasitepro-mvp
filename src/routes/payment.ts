...
router.get('/pricing', async (req: Request, res: Response): Promise<void> => {
  try {
    const options = [
      {
        credits: 1,
        price: (PRICE_PER_CREDIT / 100).toFixed(2),
        pricePerCredit: (PRICE_PER_CREDIT / 100).toFixed(2),
      },
      {
        credits: 5,
        price: ((PRICE_PER_CREDIT * 5 * 0.8) / 100).toFixed(2),
        pricePerCredit: ((PRICE_PER_CREDIT * 0.8) / 100).toFixed(2),
        savings: '20%',
      },
      {
        credits: 10,
        price: ((PRICE_PER_CREDIT * 10 * 0.7) / 100).toFixed(2),
        pricePerCredit: ((PRICE_PER_CREDIT * 0.7) / 100).toFixed(2),
        savings: '30%',
      },
    ];
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (credits >= 10) {
        pricePerCredit = Math.floor(PRICE_PER_CREDIT * 0.7); // 30% off
      } else if (credits >= 5) {
        pricePerCredit = Math.floor(PRICE_PER_CREDIT * 0.8); // 20% off
      }
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (paymentIntent.metadata.userId !== userId) {
        res.status(403).json({ 
          error: 'Unauthorized',
          details: { message: 'Payment does not belong to this user' }
        });
        return;
      }
      ...
      if (existingPayment.rows.length > 0) {
        res.status(400).json({ 
          ...
          details: { message: 'This payment has already been credited' }
        });
        return;
      }

      // Add credits and record payment in transaction
      const result = await withTransaction(async (client) => {
        // Add credits to user
        await client.query(
          'UPDATE users SET image_credits = image_credits + $1 WHERE id = $2',
          [credits, userId]
        );
    ...
    try {
      ...
      switch (event.type) {
        case 'payment_intent.succeeded':
          ...
          // (credits are already added via /confirm endpoint)
          break;
        ...
        case 'payment_intent.payment_failed':
          ...
          await pool.query(
            ...
            [
              ...
              failedIntent.id,
              failedIntent.amount,
              failedIntent.currency,
              parseInt(failedIntent.metadata.credits || '0'),
              'failed',
            ]
          );
          break;
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT 
          ...
          credits_purchased, status, created_at
         FROM payments
         WHERE user_id = $1
