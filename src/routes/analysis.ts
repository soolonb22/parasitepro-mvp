  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!req.file) {
        res.status(400).json({ 
          error: 'Upload failed',
          details: { message: 'No image file uploaded' }
        });
        return;
      }
      ...
      if (userResult.rows.length === 0) {
        res.status(404).json({ 
          ...
          details: { message: 'User account does not exist' }
        });
        return;
      }
      ...
      if (credits < 1) {
        res.status(402).json({ 
          ...
          details: { 
            message: 'You need at least 1 credit to upload an image',
            creditsRequired: 1,
      ...
      const { url, thumbnailUrl, publicId } = await uploadImage(
        req.file.buffer,
        {
      ...
      // Create analysis record and deduct credit in transaction
      const analysisResult = await withTransaction(async (client) => {
        ...
        const result = await client.query(
          `INSERT INTO analyses (
            user_id, image_url, thumbnail_url, status, sample_type, 
            collection_date, location, uploaded_at, processing_started_at
      ...
      analyzeImage(url)
        .then(async ({ detections }) => {
          ...
          // Save detections and update analysis status
          await withTransaction(async (client) => {
            // Insert detections
            for (const detection of detections) {
              await client.query(
                `INSERT INTO detections (
                  analysis_id, parasite_id, common_name, scientific_name,
                  confidence_score, parasite_type, urgency_level, life_stage,
      ...
      if (error instanceof Error && error.message.includes('Invalid file type')) {
        res.status(400).json({ 
          ...
          details: { message: 'Only JPEG and PNG images are allowed' }
        });
        return;
      }
      ...
      res.status(500).json({ 
        error: 'Upload failed',
        details: { message: 'Internal server error during upload' }
      });
    }
  }
);
    ...
    try {
      ...
      const analysisResult = await pool.query(
        `SELECT 
          ...
          a.collection_date, a.location, a.uploaded_at, a.processing_started_at,
          a.processing_completed_at, a.user_id
         FROM analyses a
         WHERE a.id = $1`,
        [id]
      );
      ...
      if (analysisResult.rows.length === 0) {
        res.status(404).json({ 
          ...
          details: { message: 'The requested analysis does not exist' }
        });
        return;
      }
      ...
      if (analysis.user_id !== userId) {
        res.status(403).json({ 
          error: 'Unauthorized',
          details: { message: 'You do not have access to this analysis' }
        });
        return;
      }
      ...
      const detectionsResult = await pool.query(
        `SELECT 
          id, parasite_id, common_name, scientific_name, confidence_score,
      ...
      const detections = detectionsResult.rows.map((d) => ({
        ...
        boundingBox: d.bounding_box_x !== null ? {
          x: d.bounding_box_x,
          y: d.bounding_box_y,
      ...
      res.status(200).json({
        ...
        processingStartedAt: analysis.processing_started_at,
        processingCompletedAt: analysis.processing_completed_at,
        detections,
      });
    } catch (error) {
...
router.get(
  ...
  [
    ...
    query('sampleType')
      .optional()
      .isIn(['stool', 'blood', 'skin', 'other'])
      .withMessage('Invalid sample type filter'),
  ],
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const errors = validationResult(req);
      ...
      let queryText = `
        SELECT 
          a.id, a.thumbnail_url, a.status, a.sample_type,
      ...
      if (sampleType) {
        queryText += ` AND a.sample_type = $${paramIndex}`;
      ...
      if (sampleType) {
        countQuery += ` AND sample_type = $${countParamIndex}`;
        countParams.push(sampleType);
      }
